...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to http://127.0.0.1/MUJSTAYS to load the app using the recommended host and retry the flow.
        await page.goto("http://127.0.0.1/MUJSTAYS")
        
        # -> Load the application over HTTPS at https://127.0.0.1/MUJSTAYS so the app can be reached.
        ...
        # -> Navigate directly to the login page over HTTPS (https://127.0.0.1/MUJSTAYS/login.php).
        await page.goto("https://127.0.0.1/MUJSTAYS/login.php")
        
        # -> Fill the email field with owner@mujstays.com, fill the password field with Owner@1234, then click Sign In to submit the login form.
        frame = context.pages[-1]
        # Input text
        ...
        frame = context.pages[-1]
        assert await frame.locator("xpath=//*[contains(., 'Please fill in all required fields')]").nth(0).is_visible(), "The form should show a required-field validation error after attempting to proceed without required basic property details."
